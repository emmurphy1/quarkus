...
Hibernate ORM is the most widely used JPA implementation. It provides all of the features of an ORM but it is not easy use Hibernate for the simple operations that make up the majority of persistence coding requirements.  For a simple implementation with the most common data mappings you can use the {ProductName} Panache JPA extension to write more concise code.
...
This section lists common Panache repository pattern operations. These  examples use a regular JPA `Person` entity and the `PersonRepository` class which implements `PanacheRepository` interface.
...
pperson.setStatus(Status.Alive);
----
