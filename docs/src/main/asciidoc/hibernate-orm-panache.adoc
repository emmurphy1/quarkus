...
Hibernate not only takes care of the mapping from Java classes to database tables and from Java data types to SQL data types, but also provides data query and retrieval facilities. It can significantly reduce development time otherwise spent with manual data handling in SQL and Java database connectivity (JDBC). Hibernate’s design goal is to relieve you of most common data persistence-related programming tasks by eliminating the need for manual data processing using SQL and JDBC.
...
The {ProductName} Panache JPA extension eliminates the following Hibernate ORM issues:
...
=== Pananche active record pattern common operations
This section lists common Panache active record pattern operations. These examples use a `Person` entity created from the `PanacheEntity` class.
...
=== Pananche repository pattern common operations
This section lists common Panache repository pattern operations. These  examples use a regular JPA `Person` entity and the `PersonRepository` created from the `PanacheRepository` class.
